...
if protocol != 'all':
    for x in requests.get(url + protocol).text:
        if x == '\n' or x == '\r':
...
# --- Если выбран 'all', то добавить все адресса в список --- #
    ...
    for x in proticol_list[1:]:
        for x in [x for x in (y.split() for y in requests.get(url + x).text.split('\n')) if x]:
...
for x in range(len(proxy_list)):
    ...
    threading.Thread(target=functions.check, args=(proxy_list[x], protocol)).start()
    bar.next()
